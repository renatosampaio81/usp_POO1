...
# Método Fábrica para a estratégia adequada
def seleciona_estrategia(nome_arquivo):
    formato = nome_arquivo[-3:]  # seleciona os 3 últimos caracteres do nome do arquivo
    estrategias = {
        'pdf': SalvaTelaPDF,
        'jpg': SalvaTelaJPG,
        'png': SalvaTelaPNG,
...
class Tartaruga():
    ...
    def salvar(self):
        ...
        nome_arquivo = filedialog.asksaveasfilename(filetypes=TIPOS,
                                                    defaultextension='*.*')
        salvador = seleciona_estrategia(nome_arquivo)
